...
// A tabela de usuários só lista a própria conta do admin (listUsers é
// intencionalmente restrito). CLIENTE é sempre resultado de uma busca por
// e-mail (/api/admin/users/search), nunca da listagem.
const ADMIN_ROW = { id: 1, email: 'admin@allowed.test', fullName: 'Admin', role: 'admin', ativo: true, totalContas: 0 }
const CLIENTE = { id: 7, email: 'cliente@allowed.test', fullName: 'Cliente', plan: 'basico' }
...
function mockApi(overrides = {}) {
  ...
  apiFetch.mockImplementation(path => {
    ...
    if (path === '/api/admin/users') return Promise.resolve({ data: overrides.users || [ADMIN_ROW] })
...
describe('AdminPage — link de pagamento para um cliente (busca por e-mail)', () => {
  afterEach(() => vi.restoreAllMocks())

  it('busca o cliente pelo e-mail e gera o link do plano escolhido', async () => {
    const apiFetch = mockApi({
      extra: {
        '/api/admin/users/search?email=cliente%40allowed.test': () => Promise.resolve({ user: CLIENTE }),
        '/api/admin/users/7/plan-link/basico': () => Promise.resolve({ url: 'https://buy.stripe.com/xyz?client_reference_id=user%3A7' }),
      },
    })
    ...
    await userEvent.type(screen.getByLabelText('E-mail do cliente para gerar link de pagamento'), 'cliente@allowed.test')
  ...
  it('copia o link gerado para a área de transferência', async () => {
    ...
    await userEvent.type(screen.getByLabelText('E-mail do cliente para gerar link de pagamento'), 'cliente@allowed.test')
    await userEvent.click(screen.getByRole('button', { name: 'Buscar' }))
    await waitFor(() => expect(screen.getByRole('button', { name: 'Gerar link' })).toBeInTheDocument())
    await userEvent.click(screen.getByRole('button', { name: 'Gerar link' }))
    await waitFor(() => expect(screen.getByRole('button', { name: 'Copiar link' })).toBeInTheDocument())
  ...
  it('mostra erro na notificação quando o backend recusa o plano', async () => {
    mockApi({
      extra: {
        '/api/admin/users/search?email=cliente%40allowed.test': () => Promise.resolve({ user: CLIENTE }),
        '/api/admin/users/7/plan-link/basico': () => Promise.reject(new api.ApiError('Plano selecionado inválido.', 400)),
      },
    })
  ...
  it('mostra erro quando não encontra ninguém com o e-mail buscado', async () => {
    mockApi({ extra: { '/api/admin/users/search?email=ninguem%40allowed.test': () => Promise.reject(new api.ApiError('Nenhum usuário encontrado com esse e-mail.', 404)) } })

    render(<AdminPage />)
    await waitFor(() => expect(screen.getByRole('cell', { name: 'admin@allowed.test' })).toBeInTheDocument())
    await userEvent.type(screen.getByLabelText('E-mail do cliente para gerar link de pagamento'), 'ninguem@allowed.test')
    await userEvent.click(screen.getByRole('button', { name: 'Buscar' }))

    await waitFor(() => expect(screen.getByRole('alert')).toHaveTextContent('Nenhum usuário encontrado com esse e-mail.'))
...
describe('AdminPage — navegação por abas', () => {
  ...
  it('abre na aba Usuários por padrão e troca para Conciliação ao clicar', async () => {
    ...
    expect(screen.queryByRole('cell', { name: 'admin@allowed.test' })).not.toBeInTheDocument()
  })
...
describe('AdminPage — reconciliação de pagamentos não vinculados', () => {
  ...
  it('busca automaticamente pelo e-mail que a Stripe informou e vincula, removendo a linha da lista', async () => {
    const apiFetch = mockApi({
      ...
      extra: {
        '/api/admin/users/search?email=cliente%40allowed.test': () => Promise.resolve({ user: CLIENTE }),
        '/api/admin/billing/reconciliation/cs_sem_match/link': () => Promise.resolve({ status: 'paid' }),
      },
    })
    await renderNaAbaConciliacao()
    // A busca dispara sozinha porque o item traz customerEmail.
    await waitFor(() => expect(apiFetch).toHaveBeenCalledWith('/api/admin/users/search?email=cliente%40allowed.test'))
    await waitFor(() => expect(screen.getByRole('button', { name: 'Vincular' })).not.toBeDisabled())
  ...
  it('mostra erro e mantém a linha quando o backend recusa a vinculação', async () => {
    mockApi({
      ...
      extra: {
        '/api/admin/users/search?email=cliente%40allowed.test': () => Promise.resolve({ user: CLIENTE }),
        '/api/admin/billing/reconciliation/cs_sem_match/link': () => Promise.reject(new api.ApiError('Essa sessão não está marcada como paga na Stripe.', 400)),
      },
    })
    await renderNaAbaConciliacao()
    await waitFor(() => expect(screen.getByRole('button', { name: 'Vincular' })).not.toBeDisabled())
  ...
  it('permite buscar outro e-mail manualmente quando a sugestão não é a certa', async () => {
    mockApi({
      report: { unmatched: [{ ...ITEM, customerEmail: null }], checked: 1, truncated: false },
      extra: { '/api/admin/users/search?email=outro%40allowed.test': () => Promise.resolve({ user: CLIENTE }) },
    })
    await renderNaAbaConciliacao()
    await waitFor(() => expect(screen.getByLabelText('E-mail do cliente para vincular a sessão cs_sem_match')).toBeInTheDocument())

    await userEvent.type(screen.getByLabelText('E-mail do cliente para vincular a sessão cs_sem_match'), 'outro@allowed.test')
