...
const mediaFoldersRoutes = require('./routes/mediaFolders')
...
app.use((req, res, next) => {
  ...
  const csp = "default-src 'self'; base-uri 'self'; object-src 'none'; frame-ancestors 'none'; img-src 'self' data: blob: https:; media-src 'self' https: blob:; connect-src 'self' https:; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com; font-src 'self' https://fonts.gstatic.com data:; script-src 'self' 'sha256-BHrVdk+m3SmGDZOUJzhGyUvbBwDSONLXVLFgrlQoC+0=' 'sha256-StHcaWdc2/HgrGZXxKE0LfihFx9NPi1XasBlgsGWdeU=' 'sha256-y3U0zUge8Tuly9bqC6ykJdxX+F408naGdnaxNi05mGQ=' 'sha256-fV9a5JlVvRvGuhbxoUMgPZE5yr3uyPKJw7zKsq8sSOE=' 'sha256-PCiVuiJOVzTaly8PMUCHDSCNKHu9h6iIPgrZZHP45kQ=' 'sha256-c/MpL7TXH9z4WiaZ3egdDwIovDJViAz5LRujZatofRU=' 'sha256-zv7xLDnuAkFMFe41o1VjUq//tTEDdG/KyxOin3XyaW8=' 'sha256-E3L+/uNfSGakMxL/kGdU1mIm04RAULzstN5QPWnVEA8='"
...
app.get('/api/me', (req, res) => {
  ...
  res.json({ id: req.user.id, email: req.user.email, role: req.user.role, plan: req.user.plan, planActive: req.user.planActive !== false, planUnrestricted: req.user.planUnrestricted === true, allowedPlatforms: req.user.allowedPlatforms || [], fullName: req.user.fullName, avatarUrl: req.user.avatarUrl, totpEnabled: req.user.totpEnabled, notificationPreferences: req.user.notificationPreferences || {} })
})
